...
- 2026-02-24 | LR-011 | Sensory harvest prompt and parser must share the same strict schema gates (`psychologicalShift`, Traditional Chinese, sentence-length and score thresholds). | Prevent low-quality or format-drifting templates from polluting Sensory Vault. | Keep prompt contract + parser validation + harvest tests updated together.
- 2026-02-24 | LR-012 | When user requests plan completion verification/documentation, finish verification + docs updates before unrelated UI refinements. | Prevent scope drift and partial delivery against explicit acceptance criteria. | Execute acceptance checklist first, run tests, then update docs and learned rules in same changeset.
- 2026-02-25 | LR-013 | Count circuit-breaker failures only on final request failure, not intermediate retry attempts. | Prevent overly sensitive breaker trips under normal retry recovery. | Record breaker failure immediately before final throw path in `lib/llm-client.ts`.
- 2026-02-26 | LR-014 | When rendering long phase labels in shared `TabsList`, explicitly override horizontal fixed-height styles and allow trigger text wrapping. | Prevent label overflow from overlapping adjacent Settings sections (for example provider defaults cards). | Use `group-data-[orientation=horizontal]/tabs:h-auto` on list and `whitespace-normal` + `h-auto` on triggers in settings phase routing UI.
- 2026-02-26 | LR-015 | Do not auto-retry Phase 3 after final breakdown validation fails; surface the error immediately and keep prior stable content. | Prevent hidden second-pass reruns that confuse users and can overwrite expected state. | Keep `BREAKDOWN_VALIDATION_ATTEMPTS=1` and test failure path preservation in `__tests__/useStepGenerator.test.tsx`.
- 2026-02-26 | LR-016 | On analysis/breakdown errors, fallback content must use in-run snapshot buffers instead of only reading current workflow store state. | Store updates can lag/be skipped across session focus changes, causing output to disappear after completion or validation failure. | Maintain runtime latest-content buffers in `hooks/useStepGenerator.ts` and cover with regression tests for meta/chunk failure paths.
