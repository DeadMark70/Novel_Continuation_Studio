...
const BREAKDOWN_CHUNK_PROMPT =
  `你是章節框架規劃器。請請根據小說續寫大綱為指定章節範圍輸出逐章內容。
...
輸出規則：
- 【前情大綱與伏筆狀態】是過去已發生的事實，嚴禁在本次章節拆解中安排角色「重新經歷」或「重複講述」這些已發生的事件。
- 僅輸出本範圍章節，不可輸出其他章節
...
const CHAPTER_CRITICAL_ENFORCEMENT = `<critical_enforcement>
1. 【情節推進最高優先】必須完整走完 <chapter_execution_target> 的所有情節點並在指定字數內結束章節。絕對禁止卡在單一場景反覆描寫。

2. 【句長與節奏鐵律】單句絕對不可超過 58 字（特殊強調句可放寬至 65 字）。每個自然段最多只能出現 1 次破折號。情緒或感官高潮時必須切換成 8~20 字的破碎短句群；每個短句必須以標點符號（逗號「，」、句號「。」或驚嘆號「！」）結尾，嚴禁出現連續 20 字以上不含標點的文字串。
...
4. 【嚴禁維基/說明文風格】
   ...
   禁止用說明句描述設定。所有設定必須透過角色感官與動作自然呈現。

5. 【Show don't Tell 鐵律】
   禁止直接告訴讀者「她感到羞恥」「她很痛苦」。必須透過具體動作、身體反應、內心獨白來表現。

6. 【語言純度】全篇使用自然流暢的繁體中文，避免過度華麗或重複形容詞。嚴禁使用「海嘯般、浪潮、電流、無形的威壓洗禮、彷彿、如同」等抽象形而上比喻，只描寫具體物理感知。
...
【前情大綱與伏筆狀態】
- 請以流暢、帶有小說語感的敘事口吻，總結原文「已發生」的主線與關鍵支線劇情（目標長度 {{COMPRESSION_OUTLINE_TARGET_RANGE}} 字）。
- 嚴禁使用「必留事件清單」、「優化建議」、「狀態分析表」等教科書或報告體裁。這必須是一份給續寫作者看的「故事現狀回顧（The Story So Far）」。
- 劇情總結完畢後，單獨使用子標題 ` + "`【未解懸念與待回收伏筆】`" + `，精簡條列目前故事中尚未解決、留待後續處理的情節線索。
- 絕對不要給出「情節刪除/合併建議」或「寫作策略分析」，只陳述既有事實。`;
...
---
確保描述聚焦於「已建立的動態與現狀底線」，而非「未來必須發生的清單」。
請只輸出以下單一段落（不得新增其他段）：
...
- 請輸出完整五段：` +
  ...
  "`【成人元素包】`" +
  `
- 嚴禁自行編造「策略分析」、「事件清單」、「優化方案」等額外章節。
- 最後再輸出：` +
  "`【最終壓縮上下文】`" +
  `（確保整體語言風格貼近小說原本的語感，避免冰冷的機器條列式語氣。）
- 精煉列出當前時間點的角色關係狀態、核心目標與絕對底線）。
