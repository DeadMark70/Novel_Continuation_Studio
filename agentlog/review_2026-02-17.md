...
## Next Wave 優化進度（2026-02-17）

1. `NW-1 Debounced Persistence Lifecycle Flush`：✅ 完成（`useNovelStore` 新增 `hasPendingPersist/flushPendingPersist`，並掛載 `visibilitychange/pagehide` flush）。
2. `NW-2 Resilience E2E Fixture Hardening`：✅ 完成（`e2e/fixtures/analysis-contract.js` 集中維護契約合法 mock，Flow A 依 fixture 驗證）。
3. `NW-3 useStepGenerator Phase0/3 Coverage`：✅ 完成（新增 compression/breakdown 成功、錯誤、auto-resume 分支測試）。
4. `NW-4 StepOutline 可維護性重構`：✅ 完成（拆分 `useStepOutlineController` + `Outline*` 展示元件，`StepOutline.tsx` 降為編排殼層）。
